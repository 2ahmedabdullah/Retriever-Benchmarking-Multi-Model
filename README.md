...
* **Multi-Layer Routing Graph:** Constructs an advanced hierarchical network for non-exhaustive geometric traversal. 
* **Link Density (`M=32`):** Sets the maximum number of bidirectional link connections allocated for each new node. Higher values maintain graph connectivity in high-dimensional spaces.
* **Search / Build Windows (`efSearch=64`, `efConstruction=64`):** Controls the size of the dynamic candidate list evaluated during graph exploration. Matching these values at 64 ensures high accuracy while keeping indexing memory overhead perfectly optimized.
* **Retrieval Depth (`top_k=10`):** Truncates candidates exactly at the 10th closest neighbor to accurately calculate localized ranking attributes like NDCG@10, Precision@10, and Recall@10.
